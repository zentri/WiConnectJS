  ...
  return function(args) {
    ...
    getCommands.forEach(function(cmd){
      var getFn = function() {
        ...
        var uri = device.host + '/command/' + Object.keys(cmd)[0] + ((typeof args === 'string') ? ' ' + args : '');
    ...
    postCommands.forEach(function(cmd){
      var postFn = function(args, callback) {
        ...
        var xhr = request.post(uri)
                    .set('Content-Type', 'application/json')
                    .set('Accept', 'application/json')
                    .send(JSON.stringify({flags:0, command: cmd[Object.keys(cmd)[0]] + ((typeof args === 'string') ? ' ' + args : '')}));
